...
export default function HeaderAlmoco({
  ...
  // ❌ REGRA 1 — almoço FINALIZADO → NÃO MOSTRA HEADER
  if (almoco.fim) {
    return null;
  }

  // ✅ REGRA 2 — nunca iniciou OU foi suspenso → pode iniciar novamente
  if (!almoco.inicio || almoco.suspensoEm) {
    return (
      <div style={{ padding: 8, background: "#0E1A25", color: "#fff" }}>
        <button
          onClick={onIniciar}
  ...
  // ✅ REGRA 3 — almoço ATIVO
  const dur = Math.floor(
    (Date.now() - new Date(almoco.inicio)) / 60000
  );
  ...
  return (
    ...
    >
      <div>
        <strong>🍽️ Almoço iniciado:</strong> {almoco.inicio}
      </div>
      <div>
        <strong>Duração:</strong> {dur} min
      </div>
